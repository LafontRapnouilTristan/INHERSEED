...
species_list <- species_list |>
  dplyr::mutate(species=SPECIES,
         genus=gsub(" ","",stringr::str_remove(SPECIES,"(?<=\\s).*"))) |>
  dplyr::mutate(genus=ifelse(is.na(genus),species,genus))

#load family info
ordered_list <- readxl::read_xlsx("data/monitoring_and_info.xlsx",sheet = 2)  |> #
  dplyr::select(c(2:5)) |>
  dplyr::rename(species=species_for_tree)

#add family
species_list <- species_list |>
  left_join(ordered_list) |>
  unique() |>
  dplyr::mutate(species_for_tree=gsub(" ","_",species))
...
# a tree with only our species
tree_only_our_sp <- tab_new_tree |>
  ape::as.phylo() |>
  ape::drop.tip(tip=tab_new_tree$label[!tab_new_tree$label%in%sp_list$genus])
# 
...
#                    tip.label = "Impatiens balsamina", #add Impatiens balsamina to the tree
#                    edge.length = 0,
#                    where = 16) 
...
This is a very large tree and we want to remove all non necessary nodes (collapse all node that are not ancestors of one of our species). The final tree will be visualized using [ITOL](https://itol.embl.de/). We will save some information to be displayed on ITOL.

```{r}
# export to nexus for itol
phytools::writeNexus(ape::as.phylo(tab_new_tree), file="data/plant_phylogeny/nexus_plant.nxs")
phytools::writeNexus(tree_only_our_sp, file="data/plant_phylogeny/nexus_our_species.nxs")
```
...
write(itol_nodes,file="data/plant_phylogeny/itol/itol_rules/itol_collapse.txt",append=TRUE)
```

Color the leaves corresponding to our species
...
test <- test |>
  ...
  mutate(cultivated=ifelse(species_for_tree%in%filter(inherseed_species_status,CULTIVATED==1)$SPECIES,1,0))
...
write(itol_leaves_uncultivated,file="data/plant_phylogeny/itol/itol_rules/itol_leaf_color_uncultivated.txt",append=TRUE)
```

only labels for our species
...
write(itol_leaves_highlight_uncultivated,file="data/plant_phylogeny/itol/itol_rules/itol_leaf_labels_uncultivated.txt",append=TRUE)
```

Color clades our species belong to
...
write(itol_orders,file="data/plant_phylogeny/itol/itol_rules/itol_strip_order.txt",append=TRUE)
```
...
write(itol_order_highlight,file="data/plant_phylogeny/itol/itol_rules/itol_order_labels.txt",append=TRUE)
```
